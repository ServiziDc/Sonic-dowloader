...
ipcMain.handle('get-spotify-keys', () => {
  const c = loadConfig();
  return {
    clientId: c.spotifyClientId || '', // il secret non torna mai al frontend
    predefinitaAttiva: !c.spotifyClientId && !!DEFAULT_SPOTIFY_CLIENT_ID
  };
});
...
ipcMain.handle('get-lastfm-key', () => {
  const c = loadConfig();
  return { key: c.lastfmApiKey || '', predefinitaAttiva: !c.lastfmApiKey && !!DEFAULT_LASTFM_API_KEY };
});
...
ipcMain.handle('get-jamendo-id', () => {
  const c = loadConfig();
  return { id: c.jamendoClientId || '', predefinitaAttiva: !c.jamendoClientId && !!DEFAULT_JAMENDO_CLIENT_ID };
});
...
ipcMain.handle('get-discogs-token', () => {
  const c = loadConfig();
  return { token: c.discogsToken || '', predefinitaAttiva: !c.discogsToken && !!DEFAULT_DISCOGS_TOKEN };
});
...
ipcMain.handle('get-gemini-key', () => {
  const c = loadConfig();
  return { key: c.geminiApiKey || '', predefinitaAttiva: !c.geminiApiKey && !!DEFAULT_GEMINI_API_KEY };
});
...
function getToolsPath() {
  ...
  // Se non li troviamo da nessuna parte (es. build senza tools/ già pronta,
  // che li scarica al primo avvio), usiamo SEMPRE una cartella utente reale
  // e scrivibile — MAI __dirname, che nei pacchetti compilati punta dentro
  // app.asar (un file compresso di sola lettura, non una vera cartella:
  // provare a creare sottocartelle lì dentro dà errore "ENOTDIR").
...
async function scaricaSpotdl(tp) {
  ...
  // Il nome esatto del file può cambiare tra una versione e l'altra di spotDL
  // (es. "spotdl-4.5.2-win32.exe", "spotdl-win32.exe", "spotdl.exe"...). Proviamo
  // prima il pattern più specifico, poi ripieghiamo su varianti più generiche
  // invece di fallire subito se il nome esatto non corrisponde più.
  const asset =
    json.assets.find(a => /^spotdl.*win(32|64|dows).*\.exe$/i.test(a.name)) ||
    json.assets.find(a => /\.exe$/i.test(a.name) && !/mac|darwin|linux/i.test(a.name)) ||
    json.assets.find(a => /\.exe$/i.test(a.name));
  if (!asset) throw new Error('nessun eseguibile Windows trovato tra gli asset di questa release di spotdl');
...
ipcMain.handle('download-missing-tools', async () => {
  ...
  try {
    ...
    risultato.ytdlp = risultato.ffmpeg = risultato.deno = risultato.spotdl = false;
    risultato.errori.push('Impossibile preparare la cartella strumenti: ' + e.message);
    return risultato;
  }
